 ...
 */
@Service
public class StreamCandleServiceImpl implements StreamCandleService {
    ...
    public StreamCandleServiceImpl(
    ...
    public List<StreamTarget> targets(MarketType market) {
        return pairs.findEnabledOn(market).stream()
                ...
                .map(StreamCandleServiceImpl::target)
                .toList();
    }
    ...
    public boolean onKline(MarketType market, StreamTarget target, KlineMessage message) {
        if (!message.closed() || !CandleBackfillServiceImpl.TIMEFRAMES.contains(message.interval())) {
            return false;
        }
